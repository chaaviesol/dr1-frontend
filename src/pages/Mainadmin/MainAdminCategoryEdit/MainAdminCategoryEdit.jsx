...
export const MainAdminCategoryEdit = () => {
  ...
  const CheckingDataInputed = () => {
    ...
    if (Type[0] === "Features") {
      const FindSecInd = categories?.originalData?.findIndex(
        (ele) => ele?.Hospital
      );
      const originalData =
        categories?.originalData[FindSecInd]?.Hospital?.features;
      ...
      const FindDataIndex = categories?.transformedData?.findIndex(
        (ele) => ele?.type === Type[0]
      );
      console.log(FindDataIndex);
      const originalData =
        categories?.transformedData[FindDataIndex].department;
  ...
  const CheckingDataInputedLab = () => {
    ...
    console.log("Type>>>", Type);
    const FindIndex = categories?.originalData?.findIndex(
      (ele) => ele?.Laboratory
    );
    const originalData =
      categories?.originalData[FindIndex]?.Laboratory?.[Type[0].toLowerCase()];
  ...
  const HosChangeSections = (sec) => {
    ...
    if (retrunElement === "success") {
      if (sec === "Features") {
        ...
        const FindData = categories?.transformedData?.find(
          (ele) => ele?.type === sec
        );
        if (FindData) {
          setHosSections({ [sec]: true });
  ...
  const LabChangeSections = (sec) => {
    ...
    if (retrunElement === "success") {
      ...
      if (sec) {
        const LabServices = categories?.originalData?.find(
          (ele) => ele?.Laboratory
        );
  ...
  const initialApiCall  = async () => {
    try {
      ...
      const response = await axios.get(`${port}/admin/getcategory`);

      const categories = response?.data?.data;
      if (categories) {
        setcategories(categories);
        setInitialStateSetting(categories);
      }
  ...
  const setInitialStateSetting = (Data) => {
    if (Data && Array.isArray(Data.allTypes) && Data.allTypes.length > 0) {
      ...
      const FindData = Data?.transformedData?.find(
        (ele) => ele?.type === Data.allTypes[0]
      );
      setHosspecialities(FindData?.department);
      setDocspecialities(FindData?.department);
    ...
    <div className="MainAdminCategoryEdit">
      ...
      {categories?.transformedData ? (
        <>
            ...
            <div className="MainAdminCatSetSec">
              <div className="MainAdminCatSetFirstSec">
                {categories?.allTypes?.map((sec) => (
                  <div
                    onClick={() => {
                      HosChangeSections(sec);
                    }}
                    className={
                      !HosSections?.[sec]
                        ? "MainAdminCatSetFirstTile"
                        : "MainAdminCatSetFirstTileBefore"
                    }
                  >
                    <p>{sec}</p>
                  </div>
                ...
                <div
                  onClick={() => {
                    HosChangeSections("Features");
                  }}
                  className={
                    !HosSections?.["Features"]
                      ? "MainAdminCatSetFirstTile"
                      : "MainAdminCatSetFirstTileBefore"
                  }
                >
              ...
              <div className="MainAdminCatSetSecondSec">
                    ...
                    ConditionForHos?.index !== index ? (
                      <div className="MainAdminCatSetSecondSecDivIcon">
                        {ConditionForHos?.edit && (
                          <i
                            onClick={() => {
                              OpenDeletePopup(index, "Hospital", ele);
                            }}
                            class="ri-close-line"
                          ></i>
                        )}
                        <div
                          ...
                          className="MainAdminCatSetSecondSecDiv"
                        >
                          <p>{ele}</p>
                        </div>
                      </div>
                ...
                <div className="MainAdminCatEditSec">
                  {ConditionForHos.edit && (
                    <>
                      <button
                        className="MainAdminCatEditSecbtn3"
                        onClick={() => {
                          openModal("Hospital");
                        }}
                      >
                      ...
                      <button
                        className="MainAdminCatEditSecbtn2"
                        onClick={SaveHos}
                      >
                        Save
                      </button>
                  ...
                  {!ConditionForHos.edit && (
                    <button
                      className="MainAdminCatEditSecbtn1"
                      onClick={EditFn}
                    >
                      Edit
                    </button>
                    ...
                    ConditionForDoc?.index !== index ? (
                      <div className="MainAdminCatSetSecondSecDivIcon">
                        {ConditionForDoc?.edit && (
                          <i
                            onClick={() => {
                              OpenDeletePopup(index, "Doctor", ele);
                            }}
                            class="ri-close-line"
                          ></i>
                        )}
          ...
          <div className="MainAdminCategoryContent">
            ...
            <div className="MainAdminCatSetSec">
              <div className="MainAdminCatSetFirstSec">
                <div
                  onClick={() => {
                    LabChangeSections("Services");
                  }}
                  className={
                    !LabSections?.["Services"]
                      ? "MainAdminCatSetFirstTile"
                      : "MainAdminCatSetFirstTileBefore"
                  }
                >
                ...
                <div
                  onClick={() => {
                    LabChangeSections("Features");
                  }}
                  className={
                    !LabSections?.["Features"]
                      ? "MainAdminCatSetFirstTile"
                      : "MainAdminCatSetFirstTileBefore"
                  }
                >
                    ...
                    ConditionForLab?.index !== index ? (
                      <div className="MainAdminCatSetSecondSecDivIcon">
                        {ConditionForLab?.edit && (
                          <i
                            onClick={() => {
                              OpenDeletePopup(index, "Laboratory", ele);
                            }}
                            class="ri-close-line"
                          ></i>
                        )}
          ...
          <Modal
            className="MainAdminCatEditSecModal"
            open={ModalCondition?.open}
            onClose={openModal}
          >
            <>
          ...
          {DeletePopup?.delete && (
            <Modal
              className="EditQuestionsModalSec"
              open={DeletePopup?.delete}
              onClose={closePopups}
            >
              <div className="EditQuestionsModal">
                ...
                <div className="EditQuestionsModalFlex">
                  <button
                    className="EditQuestionsModalFlex1"
                    onClick={closePopups}
                  >
                    Close
                  </button>
                  <button
                    className="EditQuestionsModalFlex2"
