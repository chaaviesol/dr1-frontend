...
export const CreateCampaign = () => {
  ...
  const filterData = (categoryFilter, prodNameFilter) => {
    ...
    return currProdData.filter((product) => {
      const categoryMatch =
        !categoryFilter ||
        (product.category[0] &&
          product.category[0].toLowerCase().includes(categoryFilter));
   
      const prodNameMatch =
        !prodNameFilter ||
        (product.name &&
          product.name.toLowerCase().includes(prodNameFilter));
      return categoryMatch  && prodNameMatch;
    });
  };
  ...
  return (
    <div className={styles.container}>
      <div className={styles.upperSection}>
        ...
        <div className={styles.details}>
          <div className={styles.first}>
            <TextFieldOut name="name" refData={campaignName_Ref} label="Name" />
          </div>
      ...
      <div className={styles.tableSection}>
        <div className={styles.table} style={{fontSize:"16px",fontWeight:"300"}}>
          ...
          <div className={styles.tabData}>
            {showNoProds ? (
              ...
              (filteredProdData.length != 0 ? filteredProdData : prodData).map(
                (ele, index) => (
                  <div key={index} className={styles.Row}>
                    ...
                    <div className={styles.tabCheck}>
                      <IconButton
                        onClick={() =>
                          handleSelectSingleProd(ele.id, ele?.checked)
                        }
                        style={{ height: "40px" }}
                      >
                        <i
                    ...
                    <div className={styles.tabCat}>{ele?.category[0]}</div>
                    <div className={styles.TabProd}>{ele?.name}</div>
