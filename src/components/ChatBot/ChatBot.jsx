...
const ChatBot = () => {
    ...
    if (ChatSec) {
      ...
      if (Chats.find((ele) => ele.Qnm === 1 && ele.answer === "Doctor")) {
        ...
        Chats.find(
          (ele) =>
            ele.Qnm === 1 &&
            (ele.answer === "Hospital" || ele.answer === "Laboratory")
        )
      ) {
  ...
  const ConfirmInput = () => {
    ...
    TempChats[FInalQnmIndex] = {
      ...TempChats[FInalQnmIndex],
      answer: TempUserInput,
    };
  ...
  const UpdatingMsgs = (tmepchats) => {
    ...
    if (
      ...
      !TempChat?.find(
        (ele) =>
          ele.bot === botHos2Q || ele.bot === botLab2Q || ele.bot === botDoc2Q
      )
    ) {
      switch (who) {
        case "Doctor":
    ...
    if (findFirstQ?.answer === "Doctor") {
      ...
      const botRes3Q =
        "Thank you for providing your details. We have assigned a relationship manager for you. You will receive call from them soon.";
      const error = "Please enter a valid phone number.";
      const PatternMob = /^[6-9]\d{9}$/;
      if (
        findThirdQ &&
        findThirdQ.answer &&
        !TempChat?.find((ele) => ele.bot === botRes3Q)
      ) {
      ...
      const findThirdQ = TempChat?.find((ele) => ele?.Qnm === 3);
      const botRes3Q =
        "Would you like to receive a call from our relationship manager?.";
      const error = "Please enter a valid phone number.";
      const PatternMob = /^[6-9]\d{9}$/;
      if (
        findThirdQ &&
    ...
    const botRes3Q =
      "Thank you for providing your details. We have assigned a relationship manager for you. You will receive call from them soon.";
    const botRes3QNo =
      "Please click the button to access the registration page.";
    if (
      findFourQ &&
      findFourQ?.answer &&
      !TempChat?.find((ele) => ele.bot === botRes3QNo)
    ) {
    ...
    const CheckingNull =
      !FinalData?.contact_number || !FinalData?.name || !FinalData?.type;
    if (FinalData.type === "Doctor") {
      ...
      FinalData.type === "Hospital" ||
      FinalData.type === "Laboratory"
    ) {
    ...
    <>
      <div className="ChatBotAlignBotIcon">
        <Modal
          ...
          className={
            ChatSec
              ? "ChatBotAlignBotChatModal open"
              : "ChatBotAlignBotChatModal"
          }
        >
          <div className="ChatBotAlignBotChat">
            <div className="ChatBotChatHeader">
              ...
              <button
                ...
                className="ChatBotChatHeaderClose"
              >
                    ...
                    {ele?.bot && (
                      <>
                        ...
                        {ele?.Qnm === 4 &&
                          Chats.find(
                            (ele) =>
                              (ele.Qnm === 1 && ele?.answer === "Hospital") ||
                              ele?.answer === "Laboratory"
                          ) && (
                            <div className="ChatBotChatsSecSuggetions">
                              {ele?.Qnm === 4 &&
                              Chats.find(
                                ...
                                <div className="ChatBotChatsSecSuggetions">
                                  <button
                                    ...
                                    className="ChatBotChatsSec3QnmBtn"
                                  >
                                    Registration link
                                  </button>
                                </div>
                              ...
                              <button
                                value="Thru relationship manager"
                                disabled={
                                  Chats?.find((ele) => ele.Qnm === 5)
                                    ? true
                                    : false
        ...
        {(window.innerWidth >= 799 || !ChatSec) && (
          <div
            ...
            className={
              ChatSec
                ? "ChatBotAlignBotShape"
                : "ChatBotAlignBotShape annimation"
            }
          >
            <svg
